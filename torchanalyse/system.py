...
class System(object):
    ...
    def __init__(self, unit, onchip_mem_bw=18000, offchip_mem_bw=900, on_chip_mem_size=float('Inf'),
                 off_chip_mem_size=float('Inf'), compute_efficiency=1, memory_efficiency=1, flops=123, mxu_shape=None,
        ...
        self.energy_per_mac = unit.unit_to_raw(0.32 + 0.2, type='E')         ## Mult + Acc + Regs inside the PE (scales down from 65nm by 1.2 so 2/1.2 for 8B read)
        self.energy_per_onchip_access = unit.unit_to_raw(8.5, type='E')
        self.energy_per_offchip_access = unit.unit_to_raw(37.5, type='E')
        self.energy_per_data_byte_onchip_to_compute = unit.unit_to_raw(2, type='E')     ## 8.5pJ for 8B from 32kB. + energy to move through CBs
        self.energy_per_data_byte_core_to_core = unit.unit_to_raw(2+2, type='E')         ## Assuming a number similar to 4MB SRAM access so 
        self.energy_per_data_byte_offchip_to_onchip = unit.unit_to_raw(37.5, type='E')   ## Assume 300 from HBM, so 300/8 = 
